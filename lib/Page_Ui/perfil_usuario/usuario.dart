...
class _PerfilUsuarioState extends State<PerfilUsuario> {
  ...
  Future<void> _loadUser() async {
    try {
      ...
      if (!mounted) return;
      final Map<String, dynamic>? user = (users is List && users.isNotEmpty)
  ...
  void actualizarinfousuario(){
    getUser(context).then((users) {
      final Map<String, dynamic>? user = (users is List && users.isNotEmpty)
          ? users.first as Map<String, dynamic>
          : null;
      final String uid = user?['uid']?.toString() ?? "";
      updateUser(nombre.text, int.parse(carnetusuario.text), cedulausuario.text, uid);
      setState(() {
        tamanofoto = 200;
        ajustando = true;
        nombre.clear();
  ...
  } // hay un error lo voy corregir despues 
